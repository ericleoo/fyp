...
struct Approach2::sketch
{
	...
	sketch(int a, int b, vector<long long> v, vector<long long> w, set<pair<long long, pair<long long, long long>>> edges, bool source)
	{
		...
		{
			for (auto it : edges)
				if (vertexset1.count(it.second.first))
				{
					edgeset.insert(it);
				}
		}
		...
		{
			for (auto it : edges)
				if (vertexset2.count(it.second.second))
				{
					edgeset.insert(it);
				}
		}
...
pair<double, int> Approach2::sourcePartitioning(sketch &s)
{
	...
	for (int i = 0; i < s.vertices1.size(); i++)
		temp[i] = {cnt[s.vertices1[i]], s.vertices1[i]};
	sort(temp.begin(), temp.end());

	for (int i = 0; i < s.vertices1.size(); i++)
		s.vertices1[i] = temp[i].second;
...
pair<double, int> Approach2::destinationPartitioning(sketch &s)
{
	...
	for (int i = 0; i < s.vertices2.size(); i++)
		temp[i] = {cnt[s.vertices2[i]], s.vertices2[i]};
	sort(temp.begin(), temp.end());

	for (int i = 0; i < s.vertices2.size(); i++)
		s.vertices2[i] = temp[i].second;
...
{
	if (s.rows < w0 || s.cols < w0)
		return true;
	if (divide(s.edgeset.size(), min(s.rows, s.cols)) <= C)
		cout << "WOW\n";
...
Approach2::Approach2(string file, int and_rows, int and_cols, int or_rows, int or_cols, int outlier_rows, int outlier_cols, int depth, int modulo, int w0, double C) : K(depth), P(modulo), w0(w0), C(C)
{
	...
	*/
	...
	for (auto it : freqCount)
		edges.insert({it.second, it.first});
